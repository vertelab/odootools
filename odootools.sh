...
function _odoo_update_module() {
    $ODOO_STOP
    sudo su odoo -c "$ODOO_SERVER -c $ODOO_SERVER_CONF --database $1 --update $2 --stop-after-init"
    $ODOO_START
}
...
function _odoo_install_module() {
    ...
    sudo su odoo -c "odoo -c /etc/odoo/odoo.conf --database ${DATABASES} --init ${MODULES} --stop-after-init"
...
function _odoogitclone() {
    ...
    for PROJECT in $(echo $PROJECTS | tr "," " "); do
        ...
        cd /usr/share
        git clone -b master10 git@github.com:vertelab/$PROJECT.git
    done
    cd $CWD
}
